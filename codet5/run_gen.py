...
def main():
    ...
    if args.do_train:
        ...
        dev_dataset = {}
        global_step, best_bleu_em, best_ppl = 0, 0,1e6
            ...
            if args.do_eval:
                ...
                if args.do_eval_bleu:
                    ...
                    if dev_bleu_em > best_bleu_em:
                        # with open("/home/fdse/sjw/ISSTA22-CodeStudy/Task/Code-Summarization-Ruby/codet5/dev_bleu.txt", 'w') as file:
                        #     file.write(str(dev_bleu_em))
    ...
    if args.do_test:
        ...
        for criteria in ['best-bleu']:
            ...
            file_path = 'codet5/test_bleu.txt'
            with open(file_path, 'r') as file:
                value = file.read()
                value = float(value)
            if(test_bleu<25): 
                with open("codet5/result.txt", 'a') as file:
                     file.write(str(test_bleu)+"\n")
                if(test_bleu<value):
                    
                    print("need training")
                else:
                    with open(file_path, 'w') as file:
                        file_path = "codet5/saved_models/checkpoint-best/pytorch_model.bin"
                        if os.path.isfile(file_path):
                            os.remove(file_path)
                        source_file_path = "codet5/saved_models/checkpoint-best-bleu/pytorch_model.bin"
                        shutil.copy(source_file_path, "codet5/saved_models/checkpoint-best")
                        file.write(str(test_bleu))
