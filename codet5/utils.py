...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if is_sample:
        examples = random.sample(examples, min(4000, len(examples)))
    if split_tag == 'train':
...
def get_filenames(data_root, task, sub_task, split='', args=None):
    if task == 'concode':
        ...
        train_fn = args.train_filename
        dev_fn = args.dev_filename
        test_fn = args.test_filename
        # data_dir = '{}/{}'.format(data_root, sub_task)
        # train_fn = '{}/train01.jsonl'.format(data_dir)
