...
def test(args, model, tokenizer, device, epoch=0):
    ...
    model.train()
    predictions=[]
    with open(os.path.join(args.output_dir,"test.output"),'w') as f, open(os.path.join(args.output_dir,"test.gold"),'w') as f1:
        for ref,gold in zip(p,eval_examples):
            predictions.append(str(gold.idx)+'\t'+ref)
            f.write(str(gold.idx)+'\t'+ref+'\n')
            f1.write(str(gold.idx)+'\t'+gold.target+'\n')     
    ...
    dev_bleu=round(bleu.bleuFromMaps(goldMap, predictionMap)[0],2)
    # file_path = 'codebert/test_bleu.txt'
    ...
    #             file_path = "codebert/saved_models/java/checkpoint-best/pytorch_model.bin"
    #             if os.path.isfile(file_path):
    #                 os.remove(file_path)
    #             source_file_path = "codebert/saved_models/java/checkpoint-best-bleu/pytorch_model.bin"
    #             shutil.copy(source_file_path, "codebert/saved_models/java/checkpoint-best")
    #             file.write(str(dev_bleu))
...
def main():
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                for batch in eval_dataloader:
                    batch = tuple(t.to(device) for t in batch)
                    source_ids,target_ids,source_mask,target_mask = batch
                    #print(source_ids.shape,target_ids.shape,source_mask.shape,target_mask.sahpe)                   
    ...
    if args.do_test:
        file = os.path.join(args.output_dir, 'checkpoint-{}/pytorch_model.bin'.format("best-bleu"))
